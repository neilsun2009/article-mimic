...
class RNNSolver(object):
  """
  A RNNSolver encapsulates all the logic necessary for training
  RNN models. The RNNSolver performs stochastic gradient
  descent using different update rules defined in optim.py.
  The solver accepts both training and validataion data and labels so it can
  periodically check classification accuracy on both training and validation
  data to watch out for overfitting.
  To train a model, you will first construct a RNNSolver instance,
  ...
  solver = RNNSolver(model, data,
                  ...
                  print_every=100)
  solver.train()
  A RNNSolver works on a model object that must conform to the following
  API:
  - model.params must be a dictionary mapping string parameter names to numpy
    arrays containing parameter values.
  - model.loss(texts) must be a function that computes
    training-time loss and gradients, with the following inputs and outputs:
    Inputs:
    - texts: Array of texts for those images, of shape (N, T) where
